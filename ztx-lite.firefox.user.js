...
// ==/UserScript==
(function () {
  var inject = function (lang) {
    var ztinit = function (window, $) {
      'ZTx-Lite';
      function Init() {
        g_drawLines = true;
        PlayerStats();
        ...
        if (g_height) {
          ...
          if (2 < points.length) {
            ...
            for (i = 1; i < points.length; i += Math.max(~~(points.length / scale), 1)) {
              for (var __unmatched_306 = i / (points.length - 1) * scale, thisNode = [
              ], __unmatched_308 = - 20; 20 >= __unmatched_308; ++__unmatched_308) {
        ...
        if ( - 1 != __unmatched_57.indexOf('Android')) {
          ...
          if ('undefined' == typeof console || 'undefined' == typeof DataView || 'undefined' == typeof WebSocket || null == canvasTest || null == canvasTest.getContext || null == window.localStorage) {
            ...
            var __unmatched_143 = function () {
              ...
              for (var __unmatched_402 = new Cell( - 1, 0, 0, 32, '#5bc0de', ''), __unmatched_403 = new Cell( - 1, 0, 0, 32, '#5bc0de', ''), __unmatched_404 = '#0791ff #5a07ff #ff07fe #ffa507 #ff0774 #077fff #3aff07 #ff07ed #07a8ff #ff076e #3fff07 #ff0734 #07ff20 #ff07a2 #ff8207 #07ff0e'.split(' '), g_skinNamesC = [
              ], j = 0; j < __unmatched_404.length; ++j) {
    ...
    script.textContent = lang + '!' + ztinit + '(window,window.jQuery);';
  ...
  if (typeof GM_info === 'undefined' || GM_info.scriptHandler) {
    alert("Your browser does not support this version of ZTx-Lite. Please install the correct version.");
  } else {
    document.addEventListener('beforescriptexecute', function (e) {
      if (e.target.text.search('i18n_lang') > -1 && e.target.text.search('ZTx-Lite') == -1) {
        var text = e.target.text;
        var lang = text.slice(0, text.search("\\(function\\(") - 1);
        e.preventDefault();
        e.stopPropagation();
